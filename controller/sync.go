...
func (m *appStateManager) SyncAppState(app *v1alpha1.Application, state *v1alpha1.OperationState) {
	...
	syncResources := make([]kube.ResourceKey, 0)
	...
	for _, res := range syncOp.Resources {
		syncResources = append(syncResources, kube.ResourceKey{
			Name:      res.Name,
	...
	syncCtx, err := sync.NewSyncContext(compareResult.reconciliationResult, restConfig, m.kubectl, app.Spec.Destination.Namespace, logEntry,
		...
		sync.WithOperationSettings(syncOp.DryRun, syncOp.Prune, syncOp.IsApplyStrategy(), syncResources...),
